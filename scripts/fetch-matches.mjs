...
// Fotbalové ligy dostupné na the-odds-api.com
const SOCCER_SPORTS = [
    'soccer_epl',
...
async function main() {
    ...
    // Deduplikace podle zápasu (bereme nejlepší kurz)
    const best = new Map();
    ...
    // Seřadit podle kurzu (nejlepší nahoře)
    unique.sort((a, b) => parseFloat(b.odds) - parseFloat(a.odds));

    // Vybrat 6 zápasů – přednostně z různých lig
    const selected = [];
    const usedLeagues = new Set();

    // Nejdřív po jednom z každé ligy
    ...
    // Doplnit zbylé z jakékoliv ligy
    for (const pick of unique) {
        if (selected.length >= PICK_COUNT) break;
    ...
    // Výstupní formát (bez interních polí)
